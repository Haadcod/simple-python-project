'''The bouncing DVD logo animation. '''
...
try:
    ...
    print('install by following the instructions')
    print('https://pypi.org/project/bext')

#set constants:
WIDTH,HEIGHT=bext.size()
#we cant print to the last column on windows wothout adding a new line automatically, so reduce the width
#by one:
WIDTH-=1
...
#Key directions for logo dictionaries:
COLOR='color'
X='x'
Y='y'
DIR='direction'
...
def main():
    ...
    while True: #main program loop.
        for logo in logos:  #handle each logo in the logos list.
            ...
            #See if logo bounces off the left edges
            elif logo[X]==0 and logo[DIR]==UP_LEFT:
                logo[DIR]=UP_RIGHT
            elif logo[X]==0 and logo[DIR]==DOWN_LEFT:
                logo[DIR]=DOWN_RIGHT
            #see if logo bounces of the right edge
        ...
        for logo in logos:
            #Draw the logo at their new location:
            bext.goto(logo[X],logo[Y])
            bext.fg(logo[COLOR])
            print('DVD',end='')
        bext.goto(0,0)
